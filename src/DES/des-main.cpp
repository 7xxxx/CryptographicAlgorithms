...
int main(int argc, char **argv) {
    ...
    //Aufgabe 3
//    bitset<16> bit_vector1((ulong) 12139); //0010111101101011
//    bitset<16> bit_vector2((ulong) 62400); //1111001111000000
//    byte *bitfield1 = new byte[2];
//    byte *bitfield2 = new byte[2];
//
//    for(int i = 0, j = 15; i < bit_vector1.size(); ++i, --j) {
//        des.setBit(bitfield1, 2, i, bit_vector1[j]);
//        des.setBit(bitfield2, 2, i, bit_vector2[j]);
//    }
//
//    cout << "Bitfield1: ";
//    des.printBitField(bitfield1, 2, 16);
//    cout << "\nBitfield2: ";
//    des.printBitField(bitfield2, 2, 16);
//    cout << "\n\n";
//
//    delete[] bitfield1;
//    delete[] bitfield2;
    ...
    if (strlen(ai.key_arg) != 16) {
        ...
        for (i = 0; i < 16; i += 2) {
            if ((isxdigit(argv[2][i]) == 0) || (isxdigit(argv[2][i+1]) == 0)) {
                cerr << "ERROR: Invalid key (no hex digit)." << endl;
                exit(1);
            }
            else {
